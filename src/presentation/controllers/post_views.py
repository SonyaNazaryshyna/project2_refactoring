...
from src.infrastructure.database.repositories import DjangoLikeRepository, DjangoUserRepository
...
@api_view(["GET"])
def post_detail_get(request, post_id: str):
    svc = get_post_service()
    pid = UUID(post_id)
...
@api_view(["PATCH"])
def post_detail_edit(request, post_id: str):
    if not request.data:
        return Response({"message": "Request body is required"}, status=status.HTTP_400_BAD_REQUEST)
    try:
...
@api_view(["DELETE"])
def post_detail_delete(request, post_id: str):
    svc = get_post_service()
    pid = UUID(post_id)
...
@api_view(["POST"])
def post_like_add(request, post_id: str):
    svc = get_post_service()
    pid = UUID(post_id)
...
@api_view(["DELETE"])
def post_like_remove(request, post_id: str):
    svc = get_post_service()
    pid = UUID(post_id)
